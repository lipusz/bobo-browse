...
public class ExpandIndex {
	public static void main(String[] args) throws IOException {
		...
		Directory srcDir = FSDirectory.getDirectory(srcIdx);
		Directory targetDir = FSDirectory.getDirectory(targetIndx);
		int timesReplicatePerSeg = numReps / numsegs;
		IndexWriter writer = new IndexWriter(targetDir, new StandardAnalyzer(),
		...
		for (int i = 0; i < numsegs; ++i) {
			...
			IndexWriter subWriter = new IndexWriter(ramDir,
		...
		writer.addIndexesNoOptimize(dirList.toArray(new Directory[dirList
				.size()]));
		writer.close();
		...
		/*
		 * SegmentInfos infos=new SegmentInfos(); infos.read(targetDir);
