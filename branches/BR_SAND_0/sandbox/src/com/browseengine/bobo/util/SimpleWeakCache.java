 ...
 * 
 * Copyright (C) 2005-2008  John Wang
 *
...
/**
 * Uses WeakReference's to the values, and stores them in an 
 * LRU scheme, with constant-time operations.
 * Note that the contract for this type of cache is that the values 
 * can be cleared at any time the WeakReference's object has been cleared.
 * 
 * <p>
 ...
 * @author spackle
 */
public class SimpleWeakCache<K,V> implements CachingScheme<K,V> {
	private CachingScheme<K,WeakReference<V>> hardCache;
	
	public SimpleWeakCache(int size) {
		hardCache = new SimpleHardCache<K,WeakReference<V>>(size);
	}

	public V cacheObject(K key, V value) {
		WeakReference<V> refVal = new WeakReference<V>(value);
		WeakReference<V> oldVal = hardCache.cacheObject(key, refVal);
		if (null != oldVal) {
			return oldVal.get();
		}
		return null;
	...
	public V getObject(K key) {
		WeakReference<V> foundVal = hardCache.getObject(key);
		if (null != foundVal) {
			return foundVal.get();
		}
		return null;
	...
	public V invalidate(K key) {
		WeakReference<V> invalidVal = hardCache.invalidate(key);
		if (null != invalidVal) {
